...
### 練習ステージ＆ランクシステム ✅
キャラ別・段階別の練習問題システム。ストーリーステージをクリアしてランクを上げ、新しい練習段階を解放する。

#### 練習ステージ
- マイページの「練習する」ボタンからアクセス
- **キャラ選択 → 練習ステージ選択 → 5問クイズ → 結果** の流れ
- キャラ別にSTAGE 1〜4の練習問題を用意（ストーリーとは別の練習専用クイズデータから出題）
- 練習クイズデータ: `assets/data/practice-easy.json`, `practice-normal.json`, `practice-hard.json`, `practice-master.json`
- 制限時間: 1問15秒
...
#### ランクシステム（キャラ別）
| ランク | 昇格条件 | 解放される練習ステージ |
|---|---|---|
| Dランク | 初期状態 | 練習STAGE 1（EASY） |
| Cランク | ストーリーSTAGE 1 クリア | 練習STAGE 2（NORMAL） |
| Bランク | ストーリーSTAGE 2 クリア | 練習STAGE 3（HARD） |
| Aランク | ストーリーSTAGE 3 クリア | 練習STAGE 4（MASTER） |
| Sランク | ストーリーSTAGE 4 クリア | （全段階解放済み） |

- ランクはキャラ別に管理（ストーリーステージのクリア状況で自動判定）
- エンディング種別問わず、クリアすればランクアップ
- ランクはステージ選択画面・練習画面に表示

#### ゲームフロー
```
マイページ → 練習する → キャラ選択 → 練習ステージ選択 → 5問クイズ → 結果
マイページ → あそぶ → ヒロイン選択 → ステージ選択 → ストーリー → クイズ
```
...
| STAGE 4 | MASTER | パートナー＋ステージ3ハッピーエンド | エターナル💍/ノーマル/バッド |
...
| 素材名 | ファイル名 | サイズ | 生成プロンプト | ステータス |
|---|---|---|---|---|
| メインビジュアル | concept-art-main.png | 1024×1024 | （上記ステップ1プロンプト参照） | ✅ 生成済 |
| UIスタイルシート | concept-art-ui-sheet.png | 1024×1024 | （上記ステップ2プロンプト参照） | ✅ 生成済 |
| 背景ムードボード | concept-art-bg-mood.png | 1024×1024 | （上記ステップ3プロンプト参照） | ✅ 生成済 |

### UI・タイトル素材（Gemini API生成）
| 素材名 | ファイル名 | サイズ | 生成プロンプト | ステータス |
|---|---|---|---|---|
| タイトルロゴ | title-logo.png | 600×200 | （上記タイトルロゴプロンプト参照） | ✅ 生成済 |
| サブタイトル装飾 | title-subtitle-deco.png | 400×60 | （上記サブタイトル装飾プロンプト参照） | ✅ 生成済 |
| タイトル背景 | bg-title.png | 800×600 | （上記タイトル背景プロンプト参照） | ✅ 生成済 |
| メインボタン | btn-primary.png | 240×60 | （上記ボタン素材プロンプト参照） | ✅ 生成済 |
| サブボタン | btn-secondary.png | 240×60 | （上記ボタン素材プロンプト参照） | ✅ 生成済 |
| アクセントボタン | btn-accent.png | 240×60 | （上記ボタン素材プロンプト参照） | ✅ 生成済 |
| 戻るボタン | btn-back.png | 48×48 | （上記ボタン素材プロンプト参照） | ✅ 生成済 |
| テキストボックス | frame-textbox.png | 760×180 | （上記ダイアログ素材プロンプト参照） | ✅ 生成済 |
| ステータスパネル | frame-panel.png | 700×400 | （上記ダイアログ素材プロンプト参照） | ✅ 生成済 |
| ポップアップフレーム | frame-popup.png | 500×350 | （上記ダイアログ素材プロンプト参照） | ✅ 生成済 |
| セリフ吹き出し | frame-speech-bubble.png | 300×120 | （上記ダイアログ素材プロンプト参照） | ✅ 生成済 |
| ハートアイコン | icon-heart.png | 64×64 | （上記アイコン素材プロンプト参照） | ✅ 生成済 |
| スターアイコン | icon-star.png | 64×64 | （上記アイコン素材プロンプト参照） | ✅ 生成済 |
| タイマーアイコン | icon-timer.png | 64×64 | （上記アイコン素材プロンプト参照） | ✅ 生成済 |
| スタミナアイコン | icon-stamina.png | 64×64 | （上記アイコン素材プロンプト参照） | ✅ 生成済 |
| ロックアイコン | icon-lock.png | 64×64 | （上記アイコン素材プロンプト参照） | ✅ 生成済 |
| 設定アイコン | icon-settings.png | icon-settings.png | 64×64 | （上記アイコン素材プロンプト参照） | ✅ 生成済 |
| 音楽アイコン | icon-music.png | 64×64 | （上記アイコン素材プロンプト参照） | ✅ 生成済 |
| ポイントアイコン | icon-point.png | 64×64 | （上記アイコン素材プロンプト参照） | ✅ 生成済 |
| ハッピーエンド装飾 | ending-happy.png | 800×600 | （上記エンディング素材プロンプト参照） | ✅ 生成済 |
| ノーマルエンド装飾 | ending-normal.png | 800×600 | （上記エンディング素材プロンプト参照） | ✅ 生成済 |
| バッドエンド装飾 | ending-bad.png | 800×600 | （上記エンディング素材プロンプト参照） | ✅ 生成済 |
| トゥルーエンド装飾 | ending-true.png | 800×600 | （上記エンディング素材プロンプト参照） | ✅ 生成済 |
| パーフェクトエンド装飾 | ending-perfect.png | 800×600 | （上記エンディング素材プロンプト参照） | ✅ 生成済 |
| エターナルエンド装飾 | ending-eternal.png | 800×600 | （上記エンディング素材プロンプト参照） | ✅ 生成済 |
| 画面遷移オーバーレイ | transition-petals.png | 800×600 | （上記エフェクト素材プロンプト参照） | ✅ 生成済 |
| キラキラエフェクト | effect-sparkle.png | 256×64 | （上記エフェクト素材プロンプト参照） | ✅ 生成済 |
| 正解エフェクト | effect-correct.png | 200×200 | （上記エフェクト素材プロンプト参照） | ✅ 生成済 |
| 不正解エフェクト | effect-wrong.png | 200×200 | （上記エフェクト素材プロンプト参照） | ✅ 生成済 |
| 美咲フレーム装飾 | deco-misaki.png | 120×120 | （上記ヒロイン別装飾プロンプト参照） | ✅ 生成済 |
| 凛フレーム装飾 | deco-rin.png | 120×120 | （上記ヒロイン別装飾プロンプト参照） | ✅ 生成済 |
| ひなたフレーム装飾 | deco-hinata.png | 120×120 | （上記ヒロイン別装飾プロンプト参照） | ✅ 生成済 |
