...
### ポイント交換所（着せ替え） ✅
マイページからアクセスできるポイント交換所。クイズで獲得したポイントで着せ替えアイテムと交換できる。
...
| ID | 名前 | カテゴリ | 価格 | 説明 |
|---|---|---|---|---|
| `ribbon` | リボン🎀 | 髪飾り | 50pt | かわいいリボンの髪飾り |
| `flower_crown` | 花かんむり🌸 | 髪飾り | 100pt | 春の花をあしらったかんむり |
| `tiara` | ティアラ👑 | 髪飾り | 200pt | キラキラ輝くお姫様のティアラ |
| `heart_earrings` | ハートピアス💖 | アクセサリー | 80pt | ハート型のかわいいピアス |
| `star_brooch` | 星のブローチ⭐ | アクセサリー | 120pt | 星が輝くおしゃれなブローチ |
| `butterfly_pin` | 蝶のヘアピン🦋 | アクセサリー | 150pt | 優雅な蝶モチーフのヘアピン |
| `cat_ears` | ねこ耳🐱 | 髪飾り | 180pt | もふもふのねこ耳カチューシャ |
| `magic_wand` | 魔法のステッキ🪄 | アクセサリー | 250pt | 魔法少女のステッキ |
| `angel_wings` | 天使の羽🪽 | スペシャル | 500pt | ふわふわの天使の羽 |
| `sparkle_frame` | キラキラフレーム✨ | スペシャル | 300pt | キャラを彩るキラキラ装飾 |

#### 機能
- マイページに現在のポイント数を表示
- 交換所でカテゴリ別にアイテムをフィルタリング
- 所持アイテムの装備/解除が可能
- 装備したアイテムはマイページのキャラ画像上に表示（絵文字オーバーレイ）
- ポイントとアイテムデータはlocalStorageに保存（キー: `heartQuizExchange`）

#### 実装
- `js/exchange.js` の `ExchangeManager` クラスで管理
- アイテムは絵文字＋CSSアニメーションで表現
- 複数アイテムの同時装備に対応
...
| ステージ4（パートナー限定） | パートナーストーリー | `story4` |
...
| 2026-03-25 | ポイント交換所追加（着せ替えアイテム10種、ポイント獲得システム） | Claude |
